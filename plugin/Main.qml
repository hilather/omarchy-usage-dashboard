...
Item {
  ...
  function updateCommand(kind, agentIds) {
    var command = [Quickshell.env("HOME") + "/.local/bin/omarchy-usage-dashboard-refresh"]
  ...
  function providerEnabled(id) {
    if (["codex", "claude", "opencode-go", "grok", "gemini", "opencode", "pi", "omp", "muse", "cursor", "devin"].indexOf(id) < 0) return false
