...
with tempfile.TemporaryDirectory(prefix='usage-dashboard-demo-') as tmp:
    ...
    for offset in range(7):
        ...
        for provider,model in [('codex','gpt-4.1'),('claude','claude-sonnet-4-20250514'),('opencode-go','glm-5.3-flash'),('grok','grok-demo'),('gemini','gemini-3.8-flash'),('opencode','example-model'),('pi','example-model'),('omp','example-model'),('muse','muse-spark-1.3-contributor'),('cursor','composer-2.5'),('devin','claude-sonnet-5-medium')]:
            for index in range(rng.randint(15,50)):
                ...
                entry = c.record(f'{provider}-{offset}-{index}',provider,f'{provider}-{offset}-{index//8}',when.isoformat(),model,
                    '/demo/'+rng.choice(['website','notes','weather']),'CLI' if provider=='codex' else 'Claude Code' if provider=='claude' else 'Muse' if provider=='muse' else 'Cursor' if provider=='cursor' else 'Devin' if provider=='devin' else 'OpenCode',
